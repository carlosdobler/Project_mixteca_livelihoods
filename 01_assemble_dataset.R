...
# ASSEMBLE DATABASE --------------------------------------------------------------------------------
tibble(a = c("/home/cdobler/Documents/Insync_ciga/Research/Fieldwork_Mixteca/encuestas//254.ods",
             "/home/cdobler/Documents/Insync_ciga/Research/Fieldwork_Mixteca/encuestas//255.ods",
             "/home/cdobler/Documents/Insync_ciga/Research/Fieldwork_Mixteca/encuestas//256.ods"),
       siembra = c(T,F,T)) -> a


list.files(dir_encuestas, full.names = T)[-c(1, 261)] %>% .[254:256] %>% #.[255] %>% # 
  ...
  map_df(function(i){
    ...
    tibble(
      ...
      # BORRAR
      siembra = a[a$a == i, "siembra", drop = T],
      ...
      #          otros_cultivos = "ER: Se necesita al menos un tipo de cultivo (sí siembra)") else 
      #     
      #     select(., -siembra)},
      # 
      # # crop area
      ...
      obs4 = raw_table[raw_table$A == "5.21" ,"B", drop = T])}) %>% View()#,
      ...
      income_otra_cant_3 = if(is.na(income_otra_name_3)) NA else functions_list$income("6.104", "6.105", "6.106"),
      ...
      tibble(member = rep(seq(1:5), each = 3),
        ...
        mutate(dest = case_when(str_detect(dest, c("OAX|HUA|NOCH")) ~ "reg",
        ...
        {tibble(migr_int = nrow(filter(., dest == "int")),
                ...
                years_int = if(migr_int == 0) NA else (.) %>% filter(dest == "int") %>% filter(year == max(year)) %>% mutate(year = 2020 - year + 1) %>% pull(year),
                years_nal = if(migr_nal == 0) NA else (.) %>% filter(dest == "nal") %>% filter(year == max(year)) %>% mutate(year = 2020 - year + 1) %>% pull(year),
                years_reg = if(migr_reg == 0) NA else (.) %>% filter(dest == "reg") %>% filter(year == max(year)) %>% mutate(year = 2020 - year + 1) %>% pull(year),
      ...
      tibble(member = rep(seq(1:6), each = 3),
        ...
        {tibble(migr_r_int = nrow(filter(., dest == "int")),
                ...
                years_r_int = if(migr_r_int == 0) NA else (.) %>% filter(dest == "int") %>% filter(year_2 == max(year_2)) %>% slice(1) %>%  mutate(year_2 = year_2 - year_1 + 1) %>% pull(year_2),
                years_r_nal = if(migr_r_nal == 0) NA else (.) %>% filter(dest == "nal") %>% filter(year_2 == max(year_2)) %>% slice(1) %>% mutate(year_2 = year_2 - year_1 + 1) %>% pull(year_2),
                years_r_reg = if(migr_r_reg == 0) NA else (.) %>% filter(dest == "reg") %>% filter(year_2 == max(year_2)) %>% slice(1) %>% mutate(year_2 = year_2 - year_1 + 1) %>% pull(year_2),
...
tibble(member = raw_table[raw_table$A == "7.2" ,"B", drop = T],
       # rel = raw_table[raw_table$A == "6.1" ,"B", drop = T],
       age_rel = raw_table[raw_table$A == "6.2" ,"B", drop = T] %>% as.integer(),
       
       tibble(#member = member,
         
         dest = c(raw_table[raw_table$A == "7.3" ,"B", drop = T],
                  raw_table[raw_table$A == "7.8" ,"B", drop = T],
                  raw_table[raw_table$A == "7.13" ,"B", drop = T]),
         year = c(raw_table[raw_table$A == "7.4" ,"B", drop = T],
                  raw_table[raw_table$A == "7.9" ,"B", drop = T],
                  raw_table[raw_table$A == "7.14" ,"B", drop = T]),
         act = c(raw_table[raw_table$A == "7.5" ,"B", drop = T],
                 raw_table[raw_table$A == "7.10" ,"B", drop = T],
                 raw_table[raw_table$A == "7.15" ,"B", drop = T]))
) %>% 
  
  filter(!is.na(dest)) %>% 
  mutate(member = case_when(str_detect(member, "HERM") & age_rel < 25 ~ 1,
                            ))
